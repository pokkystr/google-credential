...
public class GoogleCredential extends Credential {
    ...
    protected TokenResponse executeRefreshToken() throws IOException {
        ...
        payload.setAudience("https://oauth2.googleapis.com/token");
        payload.setIssuedAtTimeSeconds(currentTime / 1000);
        payload.setExpirationTimeSeconds(currentTime / 1000 + 3600);
        payload.setSubject(serviceAccountUser);
        payload.put("scope", Joiner.on(' ').join(serviceAccountScopes));
        try {
            System.out.println(getTokenServerEncodedUrl());
            String assertion = JsonWebSignature.signUsingRsaSha256(serviceAccountPrivateKey, getJsonFactory(), header, payload);
//            TokenRequest request = new TokenRequest(getTransport(), getJsonFactory(), new GenericUrl("https://oauth2.googleapis.com/token"), "urn:ietf:params:oauth:grant-type:jwt-bearer");
